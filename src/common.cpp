...
template <typename T, size_t N> T choice(const T (&arr)[N]);

template <typename T, typename A> inline void Assert(A assertion);

vector<string> split(const string &s, char delim = ' ');

string join(const vector<string> &vec, const char *delim = " ");

template <typename T> string vtos(const vector<T> &vec);
...
string join(const vector<string> &vec, const char *delim) {
  if (vec.empty())
    return "{}";
  ostringstream oss;
  copy(vec.begin(), vec.end() - 1, ostream_iterator<string>(oss, delim));
  oss << vec.back();
  return oss.str();
}
...
template <typename T, size_t N> string vtos(const T (&arr)[N]) {
  if (N == 0)
    return "[]";
  ostringstream oss;
  copy(arr, arr + N - 1, ostream_iterator<T>(oss, ","));
  oss << arr[N - 1];
  return oss.str();
}

template <typename T, size_t N> T choice(T (&arr)[N]) {
  uniform_int_distribution<int> uni(0, N - 1);
  return arr[uni(gen)];
}
